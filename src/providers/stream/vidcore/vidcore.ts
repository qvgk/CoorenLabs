...
export class VidcoreParser {
  ...
  private async extractDecryptedPayload(targetUrl: string): Promise<any> {
    ...
    try {
      ...
      await page.evaluateOnNewDocument(() => {
        ...
        JSON.parse = function() {
          const parsed = originalParse.apply(this, arguments as any);
          try {
            if (parsed && typeof parsed === 'object') {
              ...
              if (str.includes('.vtt') || str.includes('.srt')) {
                ...
                if (urls) {
                  urls.forEach(u => (window as any).__interceptedTracks.push({ file: u, label: 'Extracted Sub' }));
                }
              }
            }
          } catch (e) {}
      ...
      const payload = await new Promise<any>((resolve) => {
        ...
        const timeoutTimer = setTimeout(async () => {
          if (!isResolved) {
            isResolved = true;
            console.log(`[Vidcore] Timeout reached. Collected ${collectedSources.length}/${expectedServerCount} servers.`);
            
            const hookedTracks = await page.evaluate(() => (window as any).__interceptedTracks || []).catch(() => []);
            const domTracks = await page.evaluate(() => {
              return Array.from(document.querySelectorAll('track')).filter((t:any) => t.src).map((t:any) => ({
                label: t.label || t.srclang || 'Unknown', file: t.src
              }));
            }).catch(() => []);
          ...
          if (reqUrl.includes('/sab/') && reqUrl.includes('/re/')) {
            try {
              ...
              if (Array.isArray(json) && json.length > 0 && json[0].data) {
                ...
                console.log(`[Vidcore] Intercepted Server List: ${expectedServerCount} servers available.`);
              }
            } catch(e) {}
          }

          // Catch .vtt files returning directly via network
          if ((reqUrl.includes('.vtt') || reqUrl.includes('.srt')) && !networkSubs.some(s => s.file === reqUrl)) {
            console.log(`[Vidcore] => BINGO! Caught native network subtitle: ${reqUrl.split('/').pop()}`);
        ...
        page.on('request', async (request: any) => {
          ...
          if (reqUrl.includes('.m3u8') && !collectedSources.some(s => s.originalUrl === reqUrl)) {
            ...
            if (serverList.length > 0 && collectedSources.length >= expectedServerCount) {
              ...
              console.log(`[Vidcore] Successfully extracted all ${collectedSources.length} servers & subtitles!`);
              resolve({ success: true, sources: collectedSources, tracks: allTracks });
            }
          }
        });
        ...
        const clickInterval = setInterval(async () => {
          ...
          try {
            ...
            await page.evaluate((servers) => {
              ...
              w.__clickState = w.__clickState || { serverIdx: 0, ccClicked: false, subClicked: false };
              ...
              if (!w.__clickState.ccClicked) {
                ...
                const ccBtn = btns.find(b => {
                  ...
                  return !c.includes('mui102a04j') && (t === 'cc' || a.includes('subtitle') || a.includes('caption') || c.includes('subtitle'));
                });
                if (ccBtn) {
                  ccBtn.click();
              ...
              if (!w.__clickState.subClicked) {
                const options = Array.from(document.querySelectorAll('li, [role="menuitem"], [role="option"]'));
  ...
  private formatFinalResponse(tmdbId: string, type: string, payload: any, wyzieSubs: any[], season?: string, episode?: string) {
    if (payload.error) {
      if (payload.fallbackData && payload.fallbackData.length > 0) {
        return {
          type, tmdbId, season, episode, providerName: "vidcore", subtitles: wyzieSubs,
